...
(async () => {
    ...
    if (config.poolService.enabled) {
        const poolService = new PoolService($.consensus, config.pool, config.poolService.mySqlPsw, config.poolService.mySqlHost, config.poolService.mySqlPort);
        poolService.start();
    }
    if (config.poolPayout.enabled) {
        ...
        const poolPayout = new PoolPayout($.consensus, wallet || $.wallet, config.pool, config.poolPayout.mySqlPsw, config.poolPayout.mySqlHost, config.poolPayout.mySqlPort);
        poolPayout.start();
    }
